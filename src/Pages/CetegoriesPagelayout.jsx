...
const CategoryLayout = () => {
  ...
  useEffect(() => {
    if (mainCategory) {
      const match = categories.find(cat => cat.title.toLowerCase() === decodeURIComponent(mainCategory).toLowerCase());
      if (match) {
    ...
    if (categories.length > 0) setSelectedMain(categories[0]);
  }, [mainCategory]);

  // Filter products based on selected main category
  useEffect(() => {
    if (!selectedMain) {
      setFilteredProducts([]);
      return;
    ...
    // Map category names to match Firebase product categories
    const categoryMapping = {
      ...
      'Fashion': ['Fashion'],
      'Audio Video': ['Audio', 'Video', 'Music', 'DVD'],
      'Jewelry': ['Jewelry', 'Accessories', 'Necklaces', 'Rings', 'Earrings'],
      'Paintings': ['Paintings', 'Art', 'Canvas'],
      'Statues': ['Statues', 'Sculptures', 'Idols'],
      'Books': ['Books', 'Literature', 'Educational'],
      'Clothing & More': ['Clothing', 'Fashion', 'Apparel'],
      'Home & Living': ['Home', 'Living', 'Furniture', 'Decor'],
      'Luxe': ['Luxury', 'Premium', 'Exclusive'],
      'Best Deals': ['Deals', 'Offers', 'Discounts']
    };

    const mainCategory = categoryMapping[selectedMain.title] || [];

    // Filter products that match the main category
    const filtered = products.filter(product => {
      ...
      const matchesMain = mainCategory.some(cat => 
        productCategory.includes(cat.toLowerCase()) ||
        productTitle.includes(cat.toLowerCase()) ||
        productDescription.includes(cat.toLowerCase())
      );
